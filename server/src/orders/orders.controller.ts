...
export class OrdersController {
  ...
  async create(
    ...
    if (!body.pedido) {
      throw new BadRequestException('El campo "pedido" es requerido en el formulario multipart.');
    }
    ...
    try {
      ...
      throw new BadRequestException('El campo "pedido" debe ser un string JSON válido.');
    }

    // Validate the parsed DTO
    const errors = await validate(plainToClass(CreateOrderDto, createOrderDto));
    if (errors.length > 0) {
      const errorMessages = errors.map(error => error.constraints ? Object.values(error.constraints) : []).flat();
  ...
  @Get()
  @ApiOperation({ summary: 'Obtener una lista de pedidos con paginación y filtros' })
  ...
  @Patch(':id')
  @ApiOperation({ summary: 'Actualizar datos de un pedido en estado PENDIENTE' })
  @ApiConsumes('multipart/form-data')
  @ApiResponse({ status: 200, description: 'Pedido actualizado exitosamente.' })
  @ApiResponse({ status: 403, description: 'El pedido no se puede actualizar porque no está en estado PENDIENTE.' })
  @ApiResponse({ status: 404, description: 'Pedido no encontrado.' })
  @UseInterceptors(
    FileInterceptor('imagen', {
      limits: { fileSize: MAX_FILE_SIZE_BYTES },
  ...
  async update(
    ...
    if (!body.pedido) {
      throw new BadRequestException('El campo "pedido" es requerido en el formulario multipart para la actualización.');
    }
    ...
    try {
      ...
      throw new BadRequestException('El campo "pedido" debe ser un string JSON válido para la actualización.');
    }
  ...
  @ApiOperation({ summary: 'Cambia el estado de un pedido de PENDIENTE a EN_PRODUCCION' })
  ...
  @ApiOperation({ summary: 'Cambia el estado de un pedido de EN_PRODUCCION a EN_PROCESO' })
  ...
  @ApiOperation({ summary: 'Cambia el estado de un pedido de EN_PROCESO a LISTO_PARA_ENTREGA' })
  @ApiResponse({ status: 200, description: 'Estado cambiado a LISTO_PARA_ENTREGA.' })
  @ApiResponse({ status: 403, description: 'Transición de estado no válida.' })
  ...
  @ApiOperation({ summary: 'Cambia el estado de un pedido de LISTO_PARA_ENTREGA a ENTREGADO' })
  @ApiResponse({ status: 200, description: 'Estado cambiado a ENTREGADO y bolsa liberada.' })
  @ApiResponse({ status: 403, description: 'Transición de estado no válida.' })
  ...
  @ApiResponse({ status: 200, description: 'Pedido cancelado y bolsa liberada.' })
  @ApiResponse({ status: 403, description: 'Transición de estado no válida.' })
  ...
  @Patch('cancelar')
  @ApiOperation({ summary: 'Cancela múltiples pedidos basado en una lista de IDs de bolsas' })
  @ApiResponse({ status: 200, description: 'Pedidos cancelados exitosamente.' })
  @ApiResponse({ status: 404, description: 'Alguno de los pedidos no se encontró o no era válido para cancelación.' })
  cancelMultipleOrders(@Body() cancelOrdersDto: CancelOrdersDto) {
    return this.ordersService.cancelMultipleOrders(cancelOrdersDto);
  }

  @Delete('reset')
  @ApiOperation({ summary: 'Resetea el sistema, borrando todos los pedidos y liberando todas las bolsas. ¡USAR CON PRECAUCIÓN!' })
